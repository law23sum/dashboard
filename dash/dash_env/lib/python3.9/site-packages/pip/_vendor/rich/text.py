...
from typing import (
    Any, Callable, Dict, Iterable, List, NamedTuple, Optional, TYPE_CHECKING,
    Tuple, Union
)
...
class Text(JupyterMixin):
    ...
    def apply_meta(
            self, meta: Dict[str, Any], start: int = 0,
            end: Optional[int] = None
    ) -> None:
    ...
    def __rich_measure__(
        ...
        min_text_width = (
                max(
                        cell_len(word) for word in words
    ...
    def render(self, console: "Console", end: str = "") -> Iterable["Segment"]:
        ...
        style_map = {index: get_style(span.style) for index, span in
                     enumerated_spans}
        style_map[0] = get_style(self.style)

        spans = [
                (0, False, 0),
                *((span.start, False, index) for index, span in
                  enumerated_spans),
        ...
        self._spans[:] = [
                (
                        ...
                        if span.end < max_offset
                            ...
                            span.start, min(max_offset, span.end), span.style
                            )
                )
    ...
    def split(
        ...
        if include_separator:
            lines = self.divide(
                    ...
                    re.finditer(re.escape(separator), text)
            )
        else:
            ...
            lines = Lines(
                    line for line in self.divide(flatten_spans()) if
                    line.plain != separator
            )
    ...
    def detect_indentation(self) -> int:
        ...
        _indentations = {
                len(match.group(1))
                for match in
                re.finditer(r"^( *)(.*)$", self.plain, flags=re.MULTILINE)
        }

        try:
            indentation = (
                    reduce(
                        gcd, [indent for indent in _indentations if
                              not indent % 2]
                        ) or 1
            )
        except TypeError:
