...
CmpKey = Tuple[
    int, Tuple[
        int, ...], PrePostDevType, PrePostDevType, PrePostDevType, LocalType
]
...
# easier for 3rd party sample_code to reuse
VERSION_PATTERN = r"""
    v?
    (?:
        (?:(?P<epoch>[0-9]+)!)?                           # epoch
...
def _cmpkey(
    ...
    if local is None:
        ...
        #   match exactly
        _local = tuple(
                (i, "") if isinstance(i, int) else (NegativeInfinity, i) for i
                in local
        )
