...
_PROTO_ALLOWLIST = {
        'collections.abc'  : [
                'Callable', 'Awaitable', 'Iterable', 'Iterator',
                'AsyncIterable',
...
if sys.version_info < (3, 8):
    _EXCLUDED_ATTRS |= {
            "_gorg", "__next_in_mro__", "__extra__", "__tree_hash__",
            "__args__",
            "__origin__"
    }
...
if sys.version_info >= (3, 12):
    ...
    if sys.version_info >= (3, 8):
        ...
        class Protocol(metaclass=_ProtocolMeta):
            # There is quite a lot of overlapping sample_code with typing.Generic.
...
if sys.version_info >= (3, 8):
    ...
    def runtime_checkable(cls):
        ...
        """
        if not (
                (isinstance(cls, _ProtocolMeta) or issubclass(
                        cls, typing.Generic
                        ))
    ...
    def get_args(tp):
        ...
        if isinstance(tp, (typing._GenericAlias, _typing_GenericAlias)):
            if getattr(tp, "_special", False):
                return ()
            res = tp.__args__
            if get_origin(tp) is collections.abc.Callable and res[
                0] is not Ellipsis:
...
# 3.7-3.9
    ...
    class ParamSpec(list, _DefaultMixin):
        ...
        def __init__(
                self, name, *, bound=None, covariant=False,
                contravariant=False,
                infer_variance=False, default=_marker
                ):
            super().__init__([self])
...
if not hasattr(typing, 'Concatenate'):
    ...
    class _ConcatenateGenericAlias(list):
        ...
        def __parameters__(self):
            return tuple(
                    tp for tp in self.__args__ if
                    isinstance(tp, (typing.TypeVar, ParamSpec))
            )
...
# 3.9
    ...
    def TypeGuard(self, parameters):
        ...
        program's sample_code flow.  Usually type narrowing is done by analyzing
        conditional sample_code flow and applying the narrowing to a block of sample_code.  The
...
if hasattr(typing, "assert_never"):
    ...
    def assert_never(__arg: Never) -> Never:
        """Assert to the type checker that a line of sample_code is unreachable.
        ...
        """
        raise AssertionError("Expected sample_code to be unreachable")
...
if sys.version_info >= (3, 13):
    ...
    def _make_nmtuple(name, types, module, defaults=()):
        ...
        annotations = {n: typing._type_check(
        ...
        if __fields is _marker or __fields is None:
            warnings.warn(
                    deprecation_msg.format(
                        name=deprecated_thing, remove="3.15"
                        ),
                    DeprecationWarning,
                    stacklevel=2,
            )
...
if hasattr(collections.abc, "Buffer"):
    ...
    class Buffer(abc.ABC):
        ...
        to implement the buffer protocol in pure Python sample_code, or even
        to check whether a class implements the buffer protocol. In
        Python 3.12 and higher, the ``__buffer__`` method allows access
        to the buffer protocol from Python sample_code, and the
