...
from .core import (
    Keyword, ParseException, ParserElement, __compat__, __diag__
)
    ...
    class reset_pyparsing_context:
        ...
        def save(self):
            ...
            self._save_context["__diag__"] = {
                    name: getattr(__diag__, name) for name in
                    __diag__._all_names
            }
        ...
        def assertRunTestResults(
            ...
            if expected_parse_results is not None:
                merged = [
                        (*rpt, expected)
                        for rpt, expected in
                        zip(run_test_results, expected_parse_results)
                ]
                for test_string, result, expected in merged:
                    ...
                    expected_exception = next(
                            (
                                    exp
                                    for exp in expected
                                    if isinstance(exp, type) and issubclass(
                                exp, Exception
                                )
                            ),
                            None,
                    ...
                    if expected_exception is not None:
                        ...
                        expected_list = next(
                                (exp for exp in expected if
                                 isinstance(exp, list)), None
                        )
                        expected_dict = next(
                                (exp for exp in expected if
                                 isinstance(exp, dict)), None
                        )
    ...
    def with_line_numbers(
        ...
        if mark_control is not None:
            ...
            if mark_control == "unicode":
                transtable_map = {
                        c: u for c, u in
                        zip(range(0, 33), range(0x2400, 0x2433))
                }
        ...
        if mark_control != "unicode":
            ...
            s_lines = [line + "␊" for line in
                       s.split("␊")[start_line - 1: end_line]]
        if not s_lines:
            return ""
