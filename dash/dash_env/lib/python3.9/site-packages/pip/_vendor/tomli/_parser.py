...
def loads(__s: str, *, parse_float: ParseFloat = float) -> dict[
    str, Any]:  # noqa: C901
...
def parse_array(src: str, pos: Pos, parse_float: ParseFloat) -> tuple[
    Pos, list]:
    pos += 1
    array: list = []
...
def parse_inline_table(src: str, pos: Pos, parse_float: ParseFloat) -> tuple[
    Pos, dict]:
    pos += 1
...
def parse_basic_str_escape(
    ...
    if multiline and escape_id in {"\\ ", "\\\t", "\\\n"}:
        ...
        # the docs. Error if non-whitespace is found before newline.
        if escape_id != "\\\n":
            pos = skip_chars(src, pos, TOML_WS)
            try:
                char = src[pos]
...
def parse_literal_str(src: str, pos: Pos) -> tuple[Pos, str]:
    ...
    start_pos = pos
    pos = skip_until(
            src, pos, "'", error_on=ILLEGAL_LITERAL_STR_CHARS,
            error_on_eof=True
    )
    return pos + 1, src[start_pos:pos]  # Skip ending apostrophe


def parse_multiline_str(src: str, pos: Pos, *, literal: bool) -> tuple[
    Pos, str]:
    pos += 3
    if src.startswith("\n", pos):
        pos += 1
