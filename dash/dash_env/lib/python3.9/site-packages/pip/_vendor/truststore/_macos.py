...
try:
    ...
    Security.SecTrustSetAnchorCertificatesOnly.argtypes = [SecTrustRef,
                                                           Boolean]
...
def _handle_osstatus(
    ...
    # up with a generic one that forwards the status sample_code.
...
def _bytes_to_cf_data_ref(
        value: bytes
        ) -> CFDataRef:  # type: ignore[valid-type]
...
def _cf_string_ref_to_str(
        cf_string_ref: CFStringRef
        ) -> str | None:  # type: ignore[valid-type]
    """
...
def _der_certs_to_cf_cert_array(
        certs: list[bytes]
        ) -> CFMutableArrayRef:  # type: ignore[valid-type]
