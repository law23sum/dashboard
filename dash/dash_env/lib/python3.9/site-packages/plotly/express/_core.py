            ...
            if attr_name == "size":
                ...
                if (
                    ...
                    assert len(y_out) == len(
                        trace_patch["x"]
                    ), "missing-data-handling failure in trendline sample_code"
                    trace_patch["y"] = y_out
...
def build_dataframe(args, constructor):
    ...
    if not wide_mode and missing_bar_dim and constructor == go.Bar:
        ...
        # default and we let the normal auto-orientation-sample_code do its thing later
        ...
        if not _is_continuous(df_output, args[other_dim]):
            ...
            # can use this information to override the normal auto-orientation sample_code
            if args["orientation"] is None:
