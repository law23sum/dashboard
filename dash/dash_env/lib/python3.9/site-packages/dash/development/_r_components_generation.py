...
def generate_js_metadata(pkg_data, project_shortname):
    ...
    Returns
    -------
    function_string = complete R function sample_code to provide component features
    """
...
# This method wraps sample_code within arbitrary LaTeX-like tags, which are used
...
def write_help_file(name, props, description, prefix, rpkg_data):
    ...
    # provided as R sample_code within dash-info.yaml
    ...
    if rpkg_data is not None and "r_examples" in rpkg_data:
        ...
        result = ""
        if the_ex and "sample_code" in the_ex.keys():
            result += wrap(
                "examples",
                wrap("dontrun" if the_ex.get("dontrun") else "", the_ex["sample_code"]),
            )
