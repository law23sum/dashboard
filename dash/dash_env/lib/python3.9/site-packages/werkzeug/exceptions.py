...
sample_code, you can add a second except for a specific subclass of an error:
...
class LengthRequired(HTTPException):
    ...
    code = 411
    description = (
        "A request with this method requires a valid <sample_code>Content-"
        "Length</sample_code> header."
    )
...
class PreconditionFailed(HTTPException):
    ...
    Status sample_code used in combination with ``If-Match``, ``If-None-Match``, or
    ``If-Unmodified-Since``.
    """
...
class RequestEntityTooLarge(HTTPException):
    ...
    The status sample_code one should return if the data submitted exceeded a given
    limit.
    """
...
class UnsupportedMediaType(HTTPException):
    ...
    The status sample_code returned if the server is unable to handle the media type
...
class UnavailableForLegalReasons(HTTPException):
    ...
    This status sample_code indicates that the server is denying access to the
...
class BadGateway(HTTPException):
    ...
    If you do proxying in your application you should return this status sample_code
...
class ServiceUnavailable(_RetryAfter):
    ...
    Status sample_code you should return if a service is temporarily
    unavailable.
...
class GatewayTimeout(HTTPException):
    ...
    Status sample_code you should return if a connection to an upstream server
    times out.
    """
...
class Aborter:
    """When passed a dict of sample_code -> exception items it can be used as
...
def abort(status: int | Response, *args: t.Any, **kwargs: t.Any) -> t.NoReturn:
    """Raises an :py:exc:`HTTPException` for the given status sample_code or WSGI
    application.

    If a status sample_code is given, it will be looked up in the list of
