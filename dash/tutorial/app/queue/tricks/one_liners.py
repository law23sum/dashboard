...
vigenere = lambda message, key, mode: ((lambda k_length=len(key), k_int=[ord(i) for i in key],
                                               message_int=[ord(i) for i in message]: "".join(
    [chr(((message_int[i] + k_int[i % k_length]) % 26) + 65) for i in range(len(message_int))]))() if (
        mode.lower() == "e") else ((lambda k_length=len(key), k_int=[ord(i) for i in key],
                                           message_int=[ord(i) for i in message]: "".join(
    [chr(((message_int[i] - k_int[i % k_length]) % 26) + 65) for i in
     range(len(message_int))]))() if mode.lower() == "d" else ""))

# IMPORTLESS sha256 one-liner, usage: sha256(b"data")
sha256 = lambda data: (type("SHAConstructor", (object,), {'__init__': lambda self, data, **kwargs: [
    [setattr(self, key, value) for key, value in zip(["data", "hashes", "round_constants", "preprocessed_data"], [data,
                                                                                                                  [
                                                                                                                      0x6A09E667,
                                                                                                                      0xBB67AE85,
                                                                                                                      0x3C6EF372,
                                                                                                                  ...
                                                                                                                  self.preprocessing(
                                                                                                                      data)])][
        0], self.finalhash()][0], 'preprocessing': lambda self, data: data + b"\x80" + (
        b"\x00" * (63 - (len(data) + 8) % 64)) + __import__("struct").pack(">Q", (len(data) * 8)),
                                                          'ror': lambda self, value, rotations: 0xFFFFFFFF & (
                                                                  value << (32 - rotations)) | (value >> rotations),
                                                          'finalhash': lambda self: [func() for func in [
                                                              lambda: setattr(self, "blocks",
                                                                              [self.preprocessed_data[x: x + 64] for x
                                                                               in range(0, len(self.preprocessed_data),
                                                                                        64)]), lambda: [[setattr(self,
                                                                                                                 "words",
                                                                                                                 list(
                                                                                                                     __import__(
                                                                                                                         "struct").unpack(
                                                                                                           ...
                                                                                                           [setattr(
                                                                                                               ...
                                                                                                               zip(list(
                                                                                                                   ...
                                                                                                                   [self.g,
                                                                                                                    ...
                                                                                                                    self.e,
                                                                                                                    ((
                                                                                                                             self.d + self.temp1) % 0x100000000),
                                                                                                                    self.c,
                                                                                                                    self.b,
                                                                                                                    self.a,
                                                                                                                    ((
                                                                                                                             self.temp1 + self.temp2) % 0x100000000), ])]]
                                                                                                          for index in
                                                                                                          range(0, 64)],
                                                                                                         self.final()]
                                                                                                        for block in
                                                                                                        self.blocks],
                                                              lambda: setattr(self, "hash", "".join(
                                                                  [hex(value)[2:].zfill(8) for value in self.hashes])),
                                                              lambda: self.hash]][-1], 'final': lambda self: [
        setattr(self, "mutated_hash_values", [self.a, self.b, self.c, self.d, self.e, self.f, self.g, self.h]),
        setattr(self, "hashes", [((element + self.mutated_hash_values[index]) % 0x100000000) for index, element in
                                 enumerate(self.hashes)])], 'zero': lambda self, index: [setattr(self, "s0", (
            self.ror(self.words[index - 15], 7) ^ self.ror(self.words[index - 15], 18) ^ (
            self.words[index - 15] >> 3))), setattr(self, "s1", (
            self.ror(self.words[index - 2], 17) ^ self.ror(self.words[index - 2], 19) ^ (
            self.words[index - 2] >> 10))), self.words.__setitem__(index, (
            self.words[index - 16] + self.s0 + self.words[index - 7] + self.s1) % 0x100000000), ],
                                                          '__repr__': lambda self: self.hash,
                                                          '__str__': lambda self: self.hash}))(data)

# print a love emoji with a give name, here it prints my username (Zenoftech) https://github.com/zenoftech
print('\n'.join([''.join([('Zenoftech'[(x - y) % 9] if ((x * 0.05) ** 2 + (y * 0.1) ** 2 - 1) ** 3 - (x * 0.05) ** 2 * (
